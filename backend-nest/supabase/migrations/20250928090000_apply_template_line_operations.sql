-- Create function to atomically apply template line operations
-- This procedure handles budget propagation and template deletions in a single transaction
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE t.id = template_id
      AND (t.user_id = auth.uid() OR auth.uid() IS NULL)
  ) THEN
    RAISE EXCEPTION 'Template % not found or access denied', template_id
      USING ERRCODE = 'P0001';
  END IF;
  ...
  RETURN (
    SELECT COALESCE(array_agg(DISTINCT budget_id), ARRAY[]::uuid[])
    FROM (
      SELECT unnest(v_impacted) AS budget_id
    ) aggregated
  );
END;
$$;
