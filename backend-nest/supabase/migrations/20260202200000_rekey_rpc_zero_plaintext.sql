-- Fix: rekey RPC now also zeroes plaintext columns when setting encrypted values.
-- This enforces the dual-write invariant (plaintext = 0 when encrypted is set)
-- at the database level, fixing the backfill which previously left plaintext intact.
