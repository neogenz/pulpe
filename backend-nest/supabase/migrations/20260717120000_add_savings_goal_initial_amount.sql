-- Add savings_goal.initial_amount: encrypted stock the user already saved
-- before the goal started tracking. Nullable, no FX columns (v1 is
-- account-currency only, mirrors original_target_amount's scope).
ALTER TABLE savings_goal ADD COLUMN initial_amount TEXT;

-- Migration: Update rekey_user_encrypted_data to also re-encrypt
-- initial_amount (savings_goal). Same bug class as 20260310120000: an
-- encrypted column added without a matching rekey-flow update loses its
-- value silently on the next PIN change (UPDATE never touches the column,
-- so it stays encrypted with the OLD key while every other column rotates).
