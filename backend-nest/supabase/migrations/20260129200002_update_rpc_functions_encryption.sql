-- Update SQL RPC functions to handle amount_encrypted columns
-- Part of the dual-column encryption migration (AES-256-GCM)

-- ============================================================
-- 1. Drop unused get_budget_with_rollover (migrated to TypeScript)
-- ============================================================
...
-- ============================================================
-- 2. Update create_budget_from_template to copy amount_encrypted
-- ============================================================
...
BEGIN
  ...
  FOR template_line_record IN
    SELECT tl.id, tl.amount, tl.amount_encrypted, tl.kind, tl.recurrence, tl.name, tl.description
    FROM public.template_line tl
    WHERE tl.template_id = p_template_id
    ORDER BY tl.created_at
  LOOP
    INSERT INTO public.budget_line (
      budget_id, template_line_id, amount, amount_encrypted, kind, recurrence, name
    ) VALUES (
      new_budget_id,
      template_line_record.id,
      template_line_record.amount,
      template_line_record.amount_encrypted,
...
-- ============================================================
-- 3. Update create_template_with_lines to handle amount_encrypted
-- ============================================================
  ...
  SELECT json_build_object(
    ...
    'template_lines', COALESCE(
      (SELECT json_agg(json_build_object(
        ...
        'name', tl.name,
        'amount', tl.amount,
        'amount_encrypted', tl.amount_encrypted,
        'kind', tl.kind,
...
-- ============================================================
-- 4. Update bulk_update_template_lines to handle amount_encrypted
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    SET
      ...
      END,
      amount_encrypted = CASE
        WHEN line_update ? 'amount_encrypted' THEN (line_update->>'amount_encrypted')::TEXT
        ELSE public.template_line.amount_encrypted
      END,
      kind = CASE
...
-- ============================================================
-- 5. Update apply_template_line_operations to handle amount_encrypted
-- ============================================================
  ...
  IF COALESCE(array_length(v_budget_ids, 1), 0) > 0 THEN
    ...
    FOR v_update IN
      SELECT
        ...
        line->>'amount_encrypted' AS amount_encrypted,
    ...
    LOOP
      WITH updated_budget_lines AS (
        ...
        SET
          ...
          amount_encrypted = v_update.amount_encrypted,
          kind = v_update.kind,
