...
| `generate_budgets_atomically.sql`                     | `generate_budgets_from_template`          | consecutive creation, ordered skips, per-period savings-goal exclusions, cross-user rejection, advisory-lock presence and rollback after a late failure — PUL-6                                                         |
...
| `security_definer_function_privileges.sql`            | exposed RPC modes and grants               | exact 16-function inventory, 10 invoker RPCs, legacy plan entry point closed, five hardened authenticated definers, internal helpers inaccessible, future execution opt-in                                                    |
