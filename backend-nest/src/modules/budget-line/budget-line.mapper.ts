...
export class BudgetLineMapper {
   ...
   */
  toApi(budgetLineDb: BudgetLineRow): BudgetLine {
    return {
      id: budgetLineDb.id,
   ...
   */
  toApiList(budgetLinesDb: BudgetLineRow[]): BudgetLine[] {
    return budgetLinesDb.map((budgetLine) => this.toApi(budgetLine));
  }

  /**
   * Transform create DTO (camelCase) to database insert (snake_case)
   */
  toInsert(createDto: BudgetLineCreate, budgetId?: string): BudgetLineInsert {
   ...
   */
  toUpdate(updateDto: BudgetLineUpdate): Partial<BudgetLineInsert> {
