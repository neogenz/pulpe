...
export class DebugController {
  ...
  @ApiOperation({ 
    ...
    description: 'Generate different types of errors for testing error handling' 
  })
  testError(@Param('type') type: string, @Query('message') message?: string) {
    ...
    switch (type) {
      ...
      case 'not-found':
        const error = new Error('Resource not found: ' + errorMessage);
        error.name = 'NotFoundException';
        throw error;
      
      case 'database':
        const dbError = new Error('Database connection failed: ' + errorMessage);
        dbError.name = 'DatabaseException';
        // Simulate a cause
        (dbError as any).cause = 'Connection timeout after 5000ms';
        throw dbError;
  ...
  async testServiceError(@Body() data: { shouldFail: boolean; message?: string }) {
    try {
  ...
  private async simulateServiceCall(shouldFail: boolean, message?: string): Promise<string> {
    ...
    if (shouldFail) {
      ...
      (serviceError as any).cause = cause.message;
