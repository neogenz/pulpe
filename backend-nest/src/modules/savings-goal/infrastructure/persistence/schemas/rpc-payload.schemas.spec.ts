...
describe('applySavingsGoalPlanTemplateLineSchema', () => {
  it('accepts a valid ciphertext template line update', () => {
    expect(() =>
      applySavingsGoalPlanTemplateLineSchema.parse(validTemplateLine),
    ).not.toThrow();
  });

  it('rejects an unknown key (strict)', () => {
    expect(() =>
      applySavingsGoalPlanTemplateLineSchema.parse({
        ...validTemplateLine,
        template_id: UUID,
      }),
    ).toThrow();
  ...
  it('validates a list of template line updates', () => {
    expect(
      applySavingsGoalPlanTemplateLineListSchema.parse([validTemplateLine]),
    ).toHaveLength(1);
  });
});
...
describe('RPC P0001 message constants', () => {
  ...
  it('mirror the exact strings the RPC RAISEs', () => {
    ...
    expect(PLAN_TEMPLATE_LINE_NOT_LINKED_RPC_MESSAGE).toBe(
