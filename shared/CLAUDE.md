...
pnpm run test        # vitest run (test:watch for watch mode)
```

Turbo already orders the build (`quality` has `dependsOn: ["^build"]`) — build shared by hand only when running a package's script outside turbo.
...
Schema conventions live in `.claude/rules/03-frameworks-and-libraries/shared-schemas.md`, which loads on every file here.
