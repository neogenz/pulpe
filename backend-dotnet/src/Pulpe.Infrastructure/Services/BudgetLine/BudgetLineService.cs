...
namespace Pulpe.Infrastructure.Services.BudgetLine;
...
public sealed class BudgetLineService : IBudgetLineService
{
        ...
        Guid id, BudgetLineUpdateDto dto, AuthenticatedUser user, SupabaseRestClient supabase)
    {
        ...
        if (dto.Kind.HasValue) updatePayload["kind"] = dto.Kind.Value;
        if (dto.Recurrence.HasValue) updatePayload["recurrence"] = dto.Recurrence.Value;
