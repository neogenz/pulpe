  ...
  test('should select template for budget creation', async ({
    ...
    if (createButton) {
      ...
      try {
        ...
        const hasDialog =
          (await authenticatedPage
            .locator(
              'mat-dialog-container, mat-bottom-sheet-container, .cdk-overlay-pane',
            )
            .count()) > 0;
        const hasForm =
          (await authenticatedPage
            .locator('form, .form-container, [data-testid*="form"]')
            .count()) > 0;
        const hasTemplateContent =
          (await authenticatedPage.locator('text=/template|modèle/i').count()) >
          0;
        const navigationChanged = !authenticatedPage
          .url()
          .includes('/dashboard');

        const hasTemplateWorkflow =
          hasDialog || hasForm || hasTemplateContent || navigationChanged;
      ...
      const hasPageContent =
        (await authenticatedPage.locator('main, .content, mat-card').count()) >
        0;
