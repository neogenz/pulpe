...
base.describe('Complete Profile Flow', () => {
  ...
  base(
    'should redirect returning user with existing budget to dashboard',
    async ({ page }) => {
  ...
  base(
    'should display complete profile form for first-time user',
    async ({ page }) => {
      await page.route('**/api/v1/budgets**', (route) => {
        const url = route.request().url();
  ...
  base(
    ...
    async ({ page }) => {
      ...
      await page.route(
        '**/api/v1/budget-templates/from-onboarding',
        (route) => {
          return route.fulfill({
            status: 200,
            contentType: 'application/json',
