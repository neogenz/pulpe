...
test.describe('Budget Line Deletion', () => {
  ...
  test('should show confirmation dialog and delete budget line', async ({
    ...
    await expect(authenticatedPage.getByTestId('confirm-delete-button')).toBeVisible();
    
    await budgetDetailsPage.confirmDelete();
    await expect(authenticatedPage.getByTestId('confirm-delete-button')).toBeHidden();
  });
