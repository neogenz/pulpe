...
describe('BudgetDetailsStore - User Behavior Tests', () => {
  ...
  describe('User checks envelopes and allocated transactions', () => {
    ...
    it('check-all skips transactions whose create POST is still in flight', async () => {
      // Existing line + 0 transactions; we'll fire createAllocatedTransaction
      // and call checkAllAllocatedTransactions while the create is still in flight.
      const parentLine = createMockBudgetLine({
        id: 'line-parent',
        budgetId: mockBudgetId,
        name: 'Parent line',
      ...
      // Hold the create POST open with a manual subject so the optimistic tx
      // stays in #inFlightCreateIds while we issue checkAllAllocated.
      const createSubject = new Subject<{
        data: ReturnType<typeof createMockTransaction>;
      }>();
      mockBudgetApi.createTransaction$ = vi
        .fn()
        .mockReturnValue(createSubject.asObservable());
      mockBudgetApi.checkBudgetLineTransactions$ = vi
        .fn()
        .mockReturnValue(of({ success: true, data: [] }));

      // Fire create but DO NOT await — create stays in flight.
      const createPromise = service.createAllocatedTransaction({
        budgetId: mockBudgetId,
      ...
      // Optimistic tx is now in the store under a client UUID; create is still in flight.
      const optimisticTx = service
        .budgetDetails()
        ?.transactions.find((tx) => tx.name === 'In-flight tx');
      expect(optimisticTx).toBeDefined();

      await service.checkAllAllocatedTransactions('line-parent');

      // Bulk endpoint should NOT have been called: the only candidate tx was in-flight.
      expect(mockBudgetApi.checkBudgetLineTransactions$).not.toHaveBeenCalled();

      // Drain the create so the test cleans up.
      createSubject.next({
        data: createMockTransaction({
          id: optimisticTx!.id,
          budgetId: mockBudgetId,
