...
test("expand accepts additive SQL and inert text", () => {
  const cases = ["alter table budgets add column note text", "alter table budgets add column state text not null default 'new'", "create table audit(id uuid); create index audit_id on audit(id); create policy p on audit for select using(true)", "create or replace function demo() returns text language sql as $body$ select 'DELETE FROM budgets'; -- EXECUTE ignored\n $body$", "select $$DROP TABLE budgets$$; select 'DELETE FROM budgets', \"DROP\" -- TRUNCATE budgets"];
...
test("expand fails closed on destructive or ambiguous SQL", () => {
  const cases = [["drop table budgets", /destructive/i], ["alter table budgets rename to old_budgets", /destructive/i], ["truncate budgets", /destructive/i], ["delete from budgets", /destructive/i], ["alter table budgets set schema archive", /destructive/i], ["alter table budgets disable row level security", /security/i], ["alter table budgets disable trigger all", /security/i], ["revoke select on budgets from authenticated", /security/i], ["alter table budgets alter state set not null", /destructive/i], ["alter table budgets add unsafe text not null, add safe text default 'x'", /NOT NULL.*DEFAULT/i], ["alter table budgets add unsafe text not null, alter safe set default 'x'", /NOT NULL.*DEFAULT/i], ["alter table budgets add column state text not null", /NOT NULL.*DEFAULT/i], ["do $$ begin execute 'drop table budgets'; end $$", /DO block/i], ["execute 'drop table budgets'", /EXECUTE/i], ["create or replace function demo() returns void language plpgsql as $$ begin execute 'drop table budgets'; end $$", /EXECUTE/i], ["create procedure demo() language plpgsql as $$ begin perform 1; end $$", /dollar/i], ["create procedure demo() language plpgsql as 'begin delete from budgets; end'", /procedural/i], ["create function demo() returns void language plpgsql as E'begin execute ''drop table budgets''; end'", /procedural/i], ["create function demo() returns void language plpgsql as U&'begin perform 1; end'", /procedural/i], ["select 'oops", /unclosed/i], ["/* oops", /unclosed/i], ["do $$ begin", /unclosed/i]];
