...
struct GoalProgressCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: DesignTokens.Spacing.md) {
            ...
            VStack(spacing: DesignTokens.Spacing.sm) {
                ...
                if let projected = progress.projected {
                    statRow(
                        label: "Projection à l'échéance", value: projected.asCompactCurrency(currency),
    ...
    private var layeredBar: some View {
        ZStack(alignment: .leading) {
            ...
            ProgressBarShape(progress: CGFloat(progress.confirmedFraction ?? 0))
        ...
        .accessibilityValue(
            "\(progress.achievementPercent ?? 0)% épargné, \(Int((progress.projectedFraction * 100).rounded()))% projeté"
        )
    }
