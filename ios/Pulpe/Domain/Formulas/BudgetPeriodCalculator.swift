...
enum BudgetPeriodCalculator {
    ...
    /// "5 mars - 4 avr." — a day and an abbreviated month name, so the label follows the
    /// interface **language**, never the currency. Mirrors `formatBudgetPeriod`, whose
    /// fourth parameter has always been a locale rather than a currency.
    static func formatPeriod(
        month: Int,
        year: Int,
        payDayOfMonth: Int?,
        ...
        let formatter = Formatters.dateFormatter("dMMM", in: language)
