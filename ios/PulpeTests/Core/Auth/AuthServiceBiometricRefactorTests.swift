...
struct AuthServiceBiometricRefactorTests {
    ...
    func sessionRefreshPolicy_requiresMissingPersistedSession() {
        ...
        #expect(
            !AuthService.isConfirmedTerminalSessionFailure(AuthError.sessionMissing, persistedSessionExists: true)
        )
        #expect(
            AuthService.isConfirmedTerminalSessionFailure(AuthError.sessionMissing, persistedSessionExists: false)
        )
    }
