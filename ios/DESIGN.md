...
- **Hero Surface (Dashboard — `HomeHeroCard`):** Flat, full-width projected month-end summary with no card, border or shadow. The signed projection is dominant; plan, variance and unchecked count form one compact three-column row. A single page-level mint gradient fades continuously into `homeBackground` and covers the full viewport, including when content is shorter than the screen. A 120pt monthly trajectory uses native monotone interpolation for the realized balance, a dashed future continuation, one `Aujourd’hui` marker and a subtle horizontal `Solde prévu` reference without repeated amount, axes or legend. The summary opens realized tracking; budget detail is one inline text action. Amount masking, VoiceOver summary and Dynamic Type stacking are mandatory.
