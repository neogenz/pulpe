...
**The Two-Zone Rule (iOS implementation).** Every screen with a hero is split. Top 30–35% is the **emotion zone** — `LinearGradient` filled, financial-state-keyed (`heroComfortable` / `heroTight` / `heroDeficit`). Below is the **content zone** — `Color.appBackground` (the warm sage canvas), lists and cards on top. Transition is a soft `LinearGradient` 40–60pt, never a hard cut. Screens without a hero (templates, settings) skip the emotion zone entirely. **The home dashboard (Tour 11)** uses a continuous variant: month, estimated month-end balance, plain-language comparison and unchecked count sit directly on a full-width mint surface fading into `Color.homeBackground`; activity and pointing use flat ledger sections below, not overlapping cards. Coral is reserved for a globally negative estimate; better-than-plan and under-plan states retain the existing green/amber identity and always pair color with an explicit verdict.
...
- **Hero Surface (Dashboard — `HomeHeroCard`):** Flat, full-width estimated month-end summary with no card, border or shadow. The signed estimate comes from the complete budget: unpointed forecasts remain reserved and known transactions adjust their envelopes. One plain-language verdict compares this estimate with the transaction-free plan; the unchecked count stays secondary instead of forming a competing KPI. A single page-level mint gradient fades continuously into `homeBackground` and covers the full viewport, including when content is shorter than the screen. A 120pt budget burn-down uses native monotone interpolation for the budget left after pointed outflows, then a two-point dashed connector from `Aujourd’hui` to `Fin de période`; the destination is known but no intermediate future timing is inferred. A subtle `Prévu fin de période` rule provides the final reference without adding axes, grid, legend or daily-rate extrapolation. The summary opens realized tracking; budget detail is one inline text action. Amount masking, VoiceOver summary and Dynamic Type stacking are mandatory.
