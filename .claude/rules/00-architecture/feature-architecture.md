...
### NEVER auto-provide a feature service

Feature services must be scoped to the feature, not the root injector. `@Service()` provisions at the root injector by default, so a feature service has to opt out.
...
// CORRECT - Scoped to feature, provided by its route
...
| `@Service()` (auto-provided) | Route-level providers |
|----------------------|----------------------|
...
- [ ] Is it used ONLY in this feature? Keep in feature with `@Service({ autoProvided: false })`
