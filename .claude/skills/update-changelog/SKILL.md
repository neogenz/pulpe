...
2. Prepend the SAME release object built in Step 5b (identical `version`/`iosVersion`/`date`/`platforms`/`changes` — just the `githubUrl` field is dropped, it's unused by this module) to the top of the `RELEASES` array
