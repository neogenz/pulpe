...
4. **`backend-nest/src/modules/whats-new/domain/releases-data.ts`** — curated in-app dialog for iOS users
...
iOS `MARKETING_VERSION` is independent from the product version and changes only for an approved user-facing iOS release. A release that ships only technical iOS changes increments the build number instead. iOS is not part of the Changesets fixed group (Changesets only sees npm packages). See `references/ios-release.md` for details.
