...
2. Garder les tests de route hors de `src/app` et réutiliser le routeur de test Expo quand la navigation est concernée.
...
3. Consigner les advisories sans correctif, leur chemin build-only et la condition de retrait à la prochaine release Expo.
...
| 4    | Expo check, export production, tests et audit passent ; toute advisory restante est build-only, sans correctif amont et tracée avec condition de sortie.              |
