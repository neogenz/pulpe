...
- **Date**: 2026_08_16
- **Findings**: 0 critical, 3 warning (3 corrigés), 5 minor (2 corrigés, 3 acceptés)
...
- [x] `GoalProjectionSeriesTests` vert (15 `@Test`, dont une paramétrée sur 3 durées) dans la suite complète `PulpeTests` 2165/2165 ; `pnpm test:lexicon` vert
...
- [ ] Aucun fichier de `Features/SavingsGoals` ne dépasse 500 lignes — 4 fichiers dépassent (`SavingsGoalDetailView` 706, `GoalPlanSimulatorSheet` 609, `GoalPlanApplyRecapSheet` 609, `SavingsGoalFormSheet` 538), tous antérieurs à la branche et portant déjà `// swiftlint:disable file_length`
...
| 🟢 | code | 3 | `ios/Pulpe/Features/SavingsGoals/Components/GoalProjectionChart.swift:269` | `yMin` valait `0` en dur : un solde négatif s'aplatissait contre l'axe au lieu de creuser, et s'y lisait « zéro ». Le serveur ne clampe jamais à 0 (`docs/SAVINGS.md:157`). Défaut pré-existant. | Domaine vertical déplacé sur `GoalProjectionSeries.valueDomain`, où il est testable — il restait invisible dans les computed privées de la vue. Corrigé ; test discriminant `valueDomain_digsForANegativeBalance`. |
| 🟢 | code | 1 | `ios/Pulpe/Features/SavingsGoals/Components/GoalHeroPresentation.swift:41` | `hasClosedPlanMonth` et `currentMonthPlannedAmount`, fonctions pures de `months`, vivaient en statiques du ViewModel : une struct de présentation dépendait du ViewModel du détail pour les appeler. Relevé par claude[bot], vérifié au tip. | Passées en computed properties de `SavingsGoalProgress`, comme `displayedProjection`. Corrigé ; tests déplacés dans `PulpeTests/Domain/Models/SavingsGoalProgressTests.swift`. |
| 🟢 | rot | 3 | `ios/Pulpe/Features/SavingsGoals/Components/GoalProjectionChart.swift:304` | La somme courante `cumulative` ne sert plus que de repli pour un payload sans `projectedCumulative`, mais reste calculée à chaque mois du chemin nominal. | Acceptable tant que le repli est nécessaire ; à retirer quand plus aucun client ne peut recevoir un payload sans le champ. |
