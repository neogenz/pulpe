...
### `2)` Add one final-response verifier
...
### `3)` Encode the supported boundary
...
| 2 | One command validates the same observable endpoint matrix against local, protected preview, and production URLs without exposing credentials. |
| 3 | Markdown, Markdown 404, and 406 responses vary on `Accept`; HTML keeps native RSC tokens and the unresolved upstream gap is explicit. |
