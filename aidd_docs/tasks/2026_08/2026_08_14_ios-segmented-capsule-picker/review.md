...
- [x] Boucle (retour Maxime, itéré : le geste aussi) : scroll infini par « wrap copies » — le deck rend un cycle complet en copie avant la première carte et après la dernière (`DeckSlot`, ids `wrap-<edge>-<id>`); à l'arrêt du scroll sur une copie (`onScrollPhaseChange` → `.idle`, iOS 18) la position est rebasée sans animation sur la vraie carte — contenu et peeks identiques, swap invisible; le cycle complet (et non 2 copies, v1 glitchée) absorbe les rafales de flicks qui ne passent jamais par `.idle`; `viewAligned(limitBehavior: .always)` = un tour par geste; copies non tappables et masquées de VoiceOver; « Plus tard » et le pointage de la dernière carte deviennent un tour avant d'une seule carte (copie de la première puis rebase), plus de rembobinage
