...
5. On the final local server and preview, require `Accept` and `Accept-Encoding` on negotiated Markdown/404/406 responses, allow native RSC additions on HTML 404s, preserve native RSC tokens on homepage HTML, and record Next's upstream omission of `Accept` from final homepage HTML `Vary`.
...
| 2    | `Accept` preferences and `q` values select the correct representation, `q=0` is never served, and every negotiated Markdown/404/406 final response varies on `Accept` and `Accept-Encoding`. Direct Markdown/406 responses use the exact pair; HTML 404s may retain native RSC additions. Final homepage HTML preserves Next's RSC tokens; the upstream missing `Accept` token is explicit. |
