...
| 3 | Activer `Objectif : Voyage au Japon`. | La destination du bon objectif s’ouvre avec ses données exploitables. | La destination `Voyage au Japon` affiche 600 CHF épargnés, un montant de départ de 300 CHF et la progression semée, sans erreur réseau. | PASS | [`03-goal-destination.png`](journey-screenshots/03-goal-destination.png) | Aucun. |
| 4 | Revenir au détail puis activer `Épargne lissée`. | Les occurrences du bon groupe d’épargne lissée s’ouvrent sous le titre `Épargne lissée`. | La feuille `Épargne lissée` affiche l’occurrence `Juillet 2026` de 137 CHF, sans erreur réseau. | PASS | [`04-spread-destination.png`](journey-screenshots/04-spread-destination.png) | Aucun. |

## Exécution

| Contrôle | Résultat |
|---|---|
| `testGoalAndSpreadMetadataRemainUsableAcrossAccessibilityMatrix` | PASS — 4 modes, 8 captures |
| `testGoalAndSpreadMetadataRoutesOpenExpectedDestinations` | PASS — racine et progression objectif, titre et occurrence lissée, absence d’erreur réseau |
