...
| UDID | `9DE39075-1A6D-4ECC-B066-8F04C1CBBD87` |
...
Le harness est autonome : un service local strict alimente budget, tags et occurrences ; le service d’objectif existant fournit une progression cohérente avec la même prévision liée d’août 2026 à 413 CHF. Le scénario entre par la vraie carte d’août de `BudgetsTab`, puis utilise les routes et vues de production. Les initialiseurs de production conservent leurs services partagés par défaut.
...
| Ligne budget | Accessibility 3 | Claire | Nom et métadonnée exacte `Lissé · objectif Voyage au Japon` passent sur plusieurs lignes dans le viewport ; la carte reste ouvrable. |
...
Le bundle `/tmp/pulpe-goal-spread-final-20260730-0945.xcresult` conserve dix captures XCTest : une ligne et un détail pour chacun des quatre modes, puis les deux destinations du journey.
...
| Activer l’objectif | `savingsGoalDetailRoot` affiche `Montant de départ` et la prévision liée `Août 2026, 413,00 CHF`, sans état vide ni `Connexion impossible`. |
| Activer le lissage | La feuille `Épargne lissée` affiche `Août 2026, 413,00 CHF, ici`, sans `Connexion impossible`. |
...
| SwiftLint strict sur les 3 fichiers Swift modifiés | 0 violation. |
