...
1. Resolve current Supabase, Railway and Vercel production targets read-only; verify secret **names**, production OAuth/DCR state, redirect allowlists and backup/restore availability. Record timestamps and IDs, never secret values. Confirm ordinary login, refresh and encrypted budget access with the owner's approved account scope; without that scope, report it as pending.
...
4. Present one decision packet: actual provider IDs; settings delta; version/FR-EN-DE-IT notes proposal after merge; backup and window; isolated synthetic account scope for staging and production; exact grants, one 1 CHF test expense, cleanup and revocation; public OAuth exposure; and recovery permission. Do not reuse the permanent Maestro account or its renewed credentials for MCP acceptance.
