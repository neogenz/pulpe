...
2. **Fan-out atomique** : un **seul** `INSERT … SELECT FROM jsonb_to_recordset(p_lines)` **set-based** (jamais une boucle PL/pgSQL) dans la RPC `SECURITY INVOKER` `create_budget_lines_spread` → **tout-ou-rien**, sous les droits de l'appelant et l'isolation des policies RLS existantes. Chaque tranche est chiffrée via `ENCRYPTION_PORT` **dans le repository** avant l'appel ; `spread_group_id` (uuid partagé, issu de la clé d'idempotence client ou généré serveur) ne l'est jamais. Si une Épargne liée dépasse l'échéance payDay-aware de son objectif, le trigger annule tout le groupe et retourne `ERR_SAVINGS_GOAL_LINE_OUTSIDE_HORIZON` (422).
...
**Atomicité (renforcée vs le create additif)** : `delete(source)` + `insert(N tranches)` sont **une seule** transaction dans la RPC `SECURITY INVOKER` `create_budget_lines_spread`, sous les droits de l'appelant et l'isolation des policies RLS existantes — la suppression de la source est **repliée dans la RPC** (`p_source_budget_line_id` / `p_source_transaction_id`). Le `DELETE` consomme et verrouille logiquement la source avant l'`INSERT` : un appel concurrent ou rejoué échoue avant de créer un second groupe. Toute erreur d'insertion rollbacke aussi la suppression. Après commit, chaque budget touché est recalculé une seule fois ; une erreur est signalée comme `partialFailure`, puis le cache utilisateur est invalidé.
