...
- **Arbitrages tranchés** (2 agents Plan réconciliés) :
  ...
  - WRITE = **`POST /v1/savings-goals/:id/plan`** payload line-scoped `{monthAdjustments[], templateAdjustments[]}`. **Pas de clé d'idempotence** (UPDATE-by-value, advisory lock par goal couvre le double-tap) — plus simple que le spread (INSERT). RPC set-based, tous les guards dans le WHERE (tenant/kind/`checked_at IS NULL`/période ≥ cycle courant), `isManuallyAdjusted=true`, count mismatch → RAISE diagnostique (rollback total).
  ...
  - Mois multi-lignes : `allocateMonthAmountToLines` (proportionnel cents-exact, plus-grand-reste). « Réajuster la suite » = `splitTotalPreserving(remaining, openMonths)` — généralisation cents-exact de PUL-290.
  - UX apply-on-confirm : sandbox (web `GoalPlanSimulatorStore` provider de la page ; iOS VM co-localisé dans le sheet) → récap dialog (checkbox « Mettre à jour mon Mois Type » si horizon-template touché) → écriture pessimiste → invalidation caches savings **+** budgets. Timeline verticale (grammaire lissage), pas de grid/rail. Slider global + input jumeau (1er MatSlider/Slider natif), édition par mois = champ inline (pas de drag-on-bar : Fitts). RG-002 : jamais ambre/rouge.
...
- **Backend (teammate, A-read + E)** — `/progress` sérialise `months[]` (via `buildSavingsGoalTimeline` à côté de compute) + les 2 métriques ; repo select `is_manually_adjusted`. **Write** : `POST /v1/savings-goals/:id/plan`, migration `20260706120000_apply_savings_goal_plan_pul12.sql` (RPC SECURITY DEFINER, advisory lock par goal, single UPDATE tous guards dans le WHERE — tenant/`savings_goal_id`/`kind='saving'`/`checked_at IS NULL`/période ≥ cycle courant, `is_manually_adjusted=true`, count-mismatch → RAISE diagnostique → rollback total, leg template, REVOKE/GRANT). Repo `applyPlan` chiffre via ENCRYPTION_PORT, payload RPC Zod strict + constantes messages P0001. 3 ERROR_DEFINITIONS (422/409/500). Use-case : `invalidateForUser` 1×, recalc **budgets touchés seulement**. **Gates** : `bun run quality` exit 0, 986 unit + 8 intégration live-DB + SQL guards verts.
...
- **Décision d'arbitrage clé — surfaçage horizon-template DIFFÉRÉ v1+** : `buildSavingsGoalTimeline` ne bucketise que les `budget_line` → les mois au-delà du dernier budget généré reviennent `state:'gap'` sans marqueur template. Les 3 layers (moi + 2 teammates l'ont retrouvé indépendamment) **dégradent** : chip « Pas de budget », `templateAdjustments: []`, toggle « Mois Type » masqué. **Le leg d'écriture template de la RPC reste prêt + testé** → surfacer plus tard = additif (builder shared reçoit les `template_line` liées + borne dernier budget ; nouveau select repo). Documenté `docs/SAVINGS_PLAN.md` §2/§9.
...
- **NEXT** : (1) commit + découpage PR (l'utilisateur tranche) ; (2) ré-autoriser Linear → créer les 7 issues + rescoper PUL-285 (retirer redistribution advisory, absorbée) ; (3) revue UX/DA optionnelle (task #14, ux-ui-designer, nécessite app tournante) ; (4) v1+ : surfaçage horizon-template. Hors scope avant PR : trio migrations PUL-12 dupliqué.
